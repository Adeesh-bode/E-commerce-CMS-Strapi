...
import { createLogger as createLogger$1, build as build$1 } from "vite";
...
const loadTsConfig = ({
  ...
  const configPath = ts.findConfigFile(cwd, ts.sys.fileExists, path2);
  if (!configPath) {
    return void 0;
  }
...
const resolveViteConfig = (ctx, task) => {
  ...
  const customLogger = createLogger$1();
...
const createPackageFromTemplate = async (packagePath, opts) => {
  ...
  const template = typeof templateOrResolver === "function" ? await templateOrResolver({ cwd, logger, packagePath }) : templateOrResolver;
          ...
          switch (name) {
            ...
            case "typescript": {
              ...
              if (isTypescript) {
                ...
                const { tsconfigBuildFile, tsconfigFile } = await import("./_chunks/typescript-9ff82478.mjs");
            ...
            case "eslint": {
              if (answer) {
                ...
                if (isTypescript) {
                  ...
                  const { tsconfigEslintFile } = await import("./_chunks/typescript-9ff82478.mjs");
                ...
                const { eslintIgnoreFile } = await import("./_chunks/eslint-09ff4b97.mjs");
                files.push(
                  {
                    name: ".eslintrc",
                    contents: outdent`
